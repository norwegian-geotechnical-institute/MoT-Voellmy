...
If "yes", the terrain-normal component of gravity is modified from $g \cos\theta$ to $\max(0, g \cos\theta + \kappa_{\mathbf u} \mathbf{u}^2)$, where $\kappa_{\mathbf{u}}$ is the terrain curvature in the flow direction $\mathbf{u}$. This accounts for the modification of dry friction due to centrifugal forces in curved terrain. It is recommended to include them, but in very strongly curved terrain it may be necessary to turn them off to achieve numerical stability.
...
If entrainment is enabled, the change of the terrain slope and curvature due to erosion and deposition is tracked and may induce self-channeling of the flow. Enabling this option slows code execution down significantly because the local slope angle and curvature tensor are calculated anew at every time step.
...
This parameter has been introduced in anticipation of a deposition model; it is not contained in RCF versions prior to 2024-09-10. At present, MoT-Voellmy does not use this parameter.
...
This constant multiplies the velocity-dependent part of the force exerted by the trees. It is recommended to leave it at its default value, 1 until the forest modeling component of MoT-Voellmy has been validated and calibrated against observations and, possibly, laboratory experiments.

    Modulus of rupture            (MPa)    50.0
This parameter measures the average strength of trees against breaking. Measured values vary significantly between tree species and with tree age and season. For beech, 50 MPa is a reasonable average value.

    Forest decay coefficient     (m/s)      0.15
Together with the tree diameter, this coefficient determines how quickly trees fall down (and the forest loses its braking effect) once the avalanche has broken them. With the default value, young trees and shrubs will lose their braking effect in fractions of a second while a centennial tree takes up to 5 s to fall.
...
This parameter is not used in the erosion models TJEM and AvaFrame.

    Bed friction coefficient      (-)      0.25
If the bed strength profile is declared constant, MoT-Voellmy sets this parameter to 0. If the strength profile is declared `global`, the strength is assumed to increase with depth $b_0 - z$ according to $\tau(z) = \tau(b_0) + \mu_s \rho_b (b_0 - z) g \cos\theta$, where $\mu_s$ is the bed friction coefficient specified on this line. In the case `local`, the values are read from a raster file, the name of which must be given on this line.
...
Cells in which the flow depth or the speed fall below the specified values will be considered stopped. They may, however, be remobilized if mass from neighboring cells pushes them strongly enough to overcome friction.
    ...
    Minimum momentum fraction        (%)    1.0
A stopping criterion for the entire flow simulation rather than for individual cells can be activated by choosing a suitable value for the percentage of instantaneous "quantity of movement" relative to its maximum value over the simulation so far. The "quantity of movementent" is defined as $J(t) = \int \rho_f h(x,y,t) \| \mathbf{u}(x,y,t)\| \mathrm{d}A$, thus it is like a "scalar" total flow momentum. This criterion is essentially the same as the stopping criterion in RAMMS::Avalanche. It replaces the criterion based on absolute values implemented in versions 2021-10-25, 2024-09-10 and 2025-05-20, which nevertheless is used by MoT-Voellmy 2026-04-20 if the code is run with an earlier version of the RCF. Reasonable values of the minimum momentum fraction are in the range 1–5%.
...
If the directory containing the MoT-Voellmy executable is in the user's `PATH`, the simulation can be started by the command
    <pre>MoT-Voellmy-linux.&lt;version&gt; &lt;pathname of RCF&gt;</pre>
on Linux,
    <pre>MoT-Voellmy-macOS.&lt;version&gt; &lt;pathname of RCF&gt;</pre>
on macOS systems, and with
    <pre>MoT-Voellmy-win64.&lt;version&gt;.exe &lt;pathname of RCF&gt;</pre>
on MS Windows.
